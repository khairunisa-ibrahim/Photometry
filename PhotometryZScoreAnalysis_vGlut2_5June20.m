...
BLOCKPATH = fullfile(DATAPATH,'vGlut2-3-2-200605-133619'); %Data file have to be in the \TDTMatlabSDK\Examples\ directory
...
% Quantify changes as area under the curve for pre (-5 sec) and post (5 sec)
AUC=[]; % pre, post
AUC(1,1)=trapz(mean(zall(:,ts2(1,:) < 0 & ts2(1,:) > -5)));
AUC(1,2)=trapz(mean(zall(:,ts2(1,:) > 0 & ts2(1,:) < 5)));
subplot(2,2,4);
